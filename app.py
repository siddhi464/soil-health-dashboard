...
def get_crop_recommendation_route():
    try:
        ...
        return render_template("index.html", error=_("Error in crop recommendation: ") + str(e))
...
def problem_finder():
    ...
    if not crop:
        return render_template('index.html', fertilizer_result={
            "error": _("Crop name is missing. Please select or enter a valid crop.")
        })

    if crop not in le.classes_:
        return render_template('index.html', fertilizer_result={
            "error": _("Unknown crop: '%(crop)s'. It was not seen during training. Try a different one.", crop=crop)
        })
...
def water_requirement():
    ...
    if not location or not crop:
        return render_template("index.html", error=_("Please enter both location and crop."))

    lat, lon = get_coordinates(location)
    if lat is None:
        return render_template("index.html", error=_("Location not found."))

    hourly_data = get_hourly_weather(lat, lon)
    if not hourly_data:
        return render_template("index.html", error=_("Could not fetch weather data."))
